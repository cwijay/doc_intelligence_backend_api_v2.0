...
class TestDocumentQuestions:
    ...
    async def test_01_generate_questions_first_time(self):
        ...
        if response.status_code != 201:
            ...
            if check_response.status_code == 200:
                ...
                test_doc = next((d for d in docs if d.get("filename") == TEST_DOCUMENT), None)
                if test_doc:
                    print(f"Document found: has_content={bool(test_doc.get('file_content'))}")
                else:
                    print(f"Document {TEST_DOCUMENT} not found in organization")
            
        assert response.status_code == 201, f"Failed to generate questions: {response.text}"
    ...
    async def test_02_generate_questions_when_exists(self):
        ...
        assert response.status_code == 201, f"Failed to get existing questions: {response.text}"
        ...
        print(f"✅ Returned existing {len(data['ai_questions'])} questions (not regenerated)")
    ...
    async def test_04_update_questions_direct(self):
        ...
        assert response.status_code == 200, f"Failed to update questions: {response.text}"
    ...
    async def test_05_update_questions_regenerate(self):
        ...
        assert response.status_code == 200, f"Failed to regenerate questions: {response.text}"
        ...
        if data["ai_questions"] == self.generated_questions:
            print("   Note: Regenerated questions are similar to previous ones (expected with AI)")
        else:
            print("   Note: Questions successfully regenerated with different content")
        
        print(f"✅ Regenerated {len(data['ai_questions'])} questions with custom prompt")
        for i, question in enumerate(data["ai_questions"][:3], 1):
            print(f"   Q{i}: {question[:100]}...")
    ...
    async def test_08_update_validation(self):
        ...
        if "error" in response_data:
            ...
            print(f"   Validation error properly handled: {response_data['error']['message']}")
        else:
            assert "Either 'questions' or 'prompt' must be provided" in response_data.get("detail", "")
        ...
        if "error" in response_data:
            ...
            assert "Provide either 'questions' OR 'prompt', not both" in response_data.get("detail", "")
    ...
    async def test_10_generate_after_clear(self):
        ...
        print(f"✅ Successfully generated new questions after clearing")
