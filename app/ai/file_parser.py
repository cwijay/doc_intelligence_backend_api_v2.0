...
def parse_csv_to_markdown(file_path: str) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        markdown_content += "**File Type:** CSV\n"
        ...
        for i, col in enumerate(df.columns):
            ...
            markdown_content += (
                f"- **{col}**: {data_type} ({non_null_count} non-null values)\n"
            )
    ...
    try:
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            if len(df) > 0:
                ...
                if len(df) > 50:
                    markdown_content += (
                        f"*Showing first 50 rows of {len(df)} total rows*\n\n"
                    )
...
def parse_file_with_llama_parse(file_path: str, result_type: str = "markdown") -> str:
    ...
    try:
        ...
        if result_type == "markdown":
            ...
            parsed_content = "\n\n".join(
                [clean_parsed_content(doc.text) for doc in documents]
            )
    ...
    # Handle other files with LlamaParse
        ...
        try:
            metadata = {
                ...
                "has_headers": (
                    any(page.get("pageHeaderMarkdown") for page in json_result)
                    if json_result
                    else False
                ),
                "has_footers": (
                    any(page.get("pageFooterMarkdown") for page in json_result)
                    if json_result
                    else False
                ),
                ...
                "pages_with_content": len(
                    [doc for doc in documents if doc.text.strip()]
                ),
            }
        except Exception as e:
