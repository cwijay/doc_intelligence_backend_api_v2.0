...
def validate_organization_name(name: str) -> str:
    ...
    if re.search(r"[\s\-_.]{2,}", name):
        raise ValidationError("Organization name cannot have consecutive spaces or special characters")
...
def validate_domain(domain: Optional[str]) -> Optional[str]:
    ...
    if any(invalid in domain for invalid in invalid_domains):
        raise ValidationError("Invalid domain: localhost and internal domains not allowed")
...
def validate_organization_settings(settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sensitive_keys = [
        ...
        "auth_token", "bearer_token", "jwt", "credential", "passwd"
    ]
    ...
    for key, value in settings.items():
        if not _is_json_serializable(value):
            raise ValidationError(f"Setting '{key}' contains non-JSON serializable value")
...
def validate_search_query(query: Optional[str], field_name: str = "search query") -> Optional[str]:
    """
