...
class Document(BaseModel):
    ...
    id: Optional[str] = Field(None, description="Unique document identifier (Firestore document ID)")
    ...
    parsed_storage_path: Optional[str] = Field(None, description="GCS path to parsed content (markdown)")
    parsing_metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata from document parsing (pages, etc.)")
    file_content: Optional[str] = Field(None, description="Parsed document content (markdown) - synced with GCS")
    
    # AI-generated content (stored in Firestore)
    ai_summary: Optional[str] = Field(None, description="AI-generated document summary")
    summary_metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata about summary generation (timestamp, model, etc.)")
    ai_faq: Optional[List[Dict[str, str]]] = Field(None, description="AI-generated FAQ items as list of Q&A pairs")
    faq_metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata about FAQ generation (timestamp, model, count, etc.)")
    ai_questions: Optional[List[str]] = Field(None, description="AI-generated questions as list of strings")
    questions_metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata about questions generation (timestamp, model, count, etc.)")
    
    # User tracking
    uploaded_by: str = Field(..., description="User ID who uploaded the document")
    
    # Metadata
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Document metadata and processing info")
    
    # Status tracking
    is_active: bool = Field(default=True, description="Whether document is active (for soft delete)")
    
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When document was uploaded")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="When document was last updated")
    ...
    def validate_file_content(cls, v: Optional[str]) -> Optional[str]:
        ...
        if len(v.encode('utf-8')) > max_content_size:
            raise ValueError(f"File content exceeds maximum size of {max_content_size // 1024}KB")
    ...
    def validate_ai_summary(cls, v: Optional[str]) -> Optional[str]:
        ...
        if len(v.encode('utf-8')) > max_summary_size:
            raise ValueError(f"AI summary exceeds maximum size of {max_summary_size // 1024}KB")
    ...
    @classmethod
    def validate_ai_faq(cls, v: Optional[List[Dict[str, str]]]) -> Optional[List[Dict[str, str]]]:
        ...
        for i, item in enumerate(v):
            ...
            item_size = len(item['question'].encode('utf-8')) + len(item['answer'].encode('utf-8'))
        ...
        if total_size > max_faq_size:
            raise ValueError(f"Total AI FAQ size exceeds maximum of {max_faq_size // 1024}KB")
    ...
    def validate_ai_questions(cls, v: Optional[List[str]]) -> Optional[List[str]]:
        ...
        if total_size > max_questions_size:
            raise ValueError(f"Total AI questions size exceeds maximum of {max_questions_size // 1024}KB")
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], doc_id: Optional[str] = None) -> "Document":
        ...
        try:
            ...
            for field in ['created_at', 'updated_at']:
                if field in clean_data:
                    if isinstance(clean_data[field], str):
                        try:
                            clean_data[field] = datetime.fromisoformat(clean_data[field])
                        except ValueError:
            ...
            if 'file_size' in clean_data and (clean_data['file_size'] is None or clean_data['file_size'] == ''):
            ...
            if 'file_type' in clean_data:
                if clean_data['file_type'] is None or clean_data['file_type'] == '':
                    ...
                    filename = clean_data.get('filename') or clean_data.get('original_filename')
                    if filename:
                        extracted_type = cls.extract_file_type(filename)
                        clean_data['file_type'] = extracted_type.value if extracted_type else FileType.PDF.value
                    else:
            ...
            for field in ['filename', 'original_filename', 'storage_path', 'org_id', 'uploaded_by']:
                if field in clean_data and (clean_data[field] is None or clean_data[field] == ''):
                    if field == 'filename':
                        clean_data[field] = clean_data.get('original_filename') or "unknown_file"
                    elif field == 'original_filename':
                        clean_data[field] = clean_data.get('filename') or "unknown_file"
                    elif field == 'storage_path':
                        clean_data[field] = "unknown_org/original/unknown_folder/unknown_file"
                    elif field == 'org_id':
            ...
            raise ValueError(f"Failed to create Document from Firestore data: {e}") from e
    ...
    def mark_as_parsed(self, parse_metadata: Optional[Dict[str, Any]] = None, content: Optional[str] = None):
    ...
    def questions_preview(self) -> str:
        ...
        return "\n".join(f"{i+1}. {question}" for i, question in enumerate(preview_questions))
    ...
    def clear_content(self):
        ...
        self.metadata['content_history'].append({
            'action': 'content_cleared',
            'timestamp': datetime.utcnow().isoformat()
        })
    
    def update_ai_summary(self, summary: Optional[str], generation_metadata: Optional[Dict[str, Any]] = None):
    ...
    def update_ai_faq(self, faq: Optional[List[Dict[str, str]]], generation_metadata: Optional[Dict[str, Any]] = None):
    ...
    def update_ai_questions(self, questions: Optional[List[str]], generation_metadata: Optional[Dict[str, Any]] = None):
    ...
    def generate_storage_path(self, org_name: str, folder_name: Optional[str] = None) -> str:
        """
    ...
    def can_transition_to(self, new_status: DocumentStatus) -> bool:
        ...
        valid_transitions = {
            ...
            DocumentStatus.PARSED: [DocumentStatus.FAILED],  # Can fail during later processing
