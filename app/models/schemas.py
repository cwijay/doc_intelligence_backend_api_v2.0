...
class OrganizationUpdate(BaseModel):
    ...
    _validate_domain = field_validator("domain")(OrganizationBase.validate_domain.__func__)
    _validate_settings = field_validator("settings")(OrganizationBase.validate_settings.__func__)
...
class OrganizationFilters(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Filter by organization name (partial match)")
...
class UserUpdate(BaseModel):
    ...
    _validate_username = field_validator("username")(UserBase.validate_username.__func__)
    _validate_full_name = field_validator("full_name")(UserBase.validate_full_name.__func__)
...
class FolderFilters(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Filter by folder name (partial match)")
    parent_folder_id: Optional[str] = Field(None, description="Filter by parent folder ID")
...
class DocumentUpdate(BaseModel):
    ...
    # Apply same validators as base class
    _validate_filename = field_validator("filename")(DocumentBase.validate_filename.__func__)
    _validate_metadata = field_validator("metadata")(DocumentBase.validate_metadata.__func__)
...
class DocumentResponse(DocumentBase):
    ...
    storage_path: str = Field(..., description="GCS storage path (e.g. 'Org/original/folder/file.pdf')")
    status: DocumentStatus = Field(..., description="Processing status (uploading, uploaded, parsing, parsed, failed)")
    
    # Processing and content fields
    parsed_storage_path: Optional[str] = Field(None, description="GCS path to parsed markdown content")
    parsing_metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata from document parsing engine")
    file_content: Optional[str] = Field(None, description="Parsed document content in markdown format")
    ...
    is_active: bool = Field(..., description="Whether document is active (false = soft deleted)")
...
class DocumentFilters(BaseModel):
    ...
    folder_id: Optional[str] = Field(None, description="Filter by folder ID (legacy uploads)")
    folder_path: Optional[str] = Field(None, description="Filter by folder path from storage_path (target_path uploads)")
...
class DocumentDownloadResponse(BaseModel):
    ...
    download_url: str = Field(..., description="Signed URL for downloading the document")
...
class DocumentSyncValidationResponse(BaseModel):
    ...
    sync_status: str = Field(..., description="Overall sync status: 'healthy', 'issues', 'error'")
    summary: Dict[str, int] = Field(..., description="Summary counts")
    issues: List[Dict[str, Any]] = Field(default_factory=list, description="List of sync issues found")
...
class DocumentSearchCriteria(BaseModel):
    ...
    include_inactive: bool = Field(..., description="Whether inactive documents were included")
...
class DocumentFirestoreMetadata(BaseModel):
    ...
    query_method: str = Field(..., description="Query method used (e.g., 'filename_exact_match')")
    last_updated: datetime = Field(..., description="When document was last updated in Firestore")
...
class DocumentRelationships(BaseModel):
    ...
    organization: DocumentRelationshipOrganization = Field(..., description="Organization information")
    folder: Optional[DocumentRelationshipFolder] = Field(None, description="Folder information")
    uploader: DocumentRelationshipUploader = Field(..., description="Uploader information")
...
class DocumentFirestoreResponse(BaseModel):
    ...
    firestore_metadata: DocumentFirestoreMetadata = Field(..., description="Firestore-specific metadata")
    relationships: DocumentRelationships = Field(..., description="Related entity information")
...
class DocumentFolderSearchCriteria(BaseModel):
    ...
    exact_match: bool = Field(..., description="Whether exact folder name match was used")
    include_inactive: bool = Field(..., description="Whether inactive documents were included")
    additional_filters: Dict[str, Any] = Field(default_factory=dict, description="Additional filters applied")
...
class DocumentFirestoreFolderListResponse(BaseModel):
    ...
    search_criteria: DocumentFolderSearchCriteria = Field(..., description="Search criteria used")
    documents: List[DocumentResponse] = Field(..., description="List of documents in folder")
    total: int = Field(..., description="Total number of documents in folder")
    page: int = Field(..., description="Current page number")
    per_page: int = Field(..., description="Number of items per page")
    total_pages: int = Field(..., description="Total number of pages")
    firestore_metadata: DocumentFirestoreMetadata = Field(..., description="Firestore query metadata")
...
class DocumentParseRequest(BaseModel):
    ...
    def validate_storage_path(cls, v: str) -> str:
        ...
        path_parts = v.split('/')
        if len(path_parts) < 3:
            raise ValueError("Storage path must have at least 3 parts: org/type/folder or org/type/file")
...
class DocumentParseResponse(BaseModel):
    ...
    parsed_storage_path: str = Field(..., description="GCS path where parsed content is stored")
    parsed_content: str = Field(..., description="Parsed document content in markdown format")
    ...
    gcs_metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Original file metadata from GCS"
    )
    file_info: DocumentParseFileInfo = Field(..., description="File size and type information")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When parsing was completed")
...
class DocumentParseStatus(BaseModel):
    ...
    status: str = Field(..., description="Parsing status (pending, processing, completed, failed)")
    message: Optional[str] = Field(None, description="Status message or error details")
    progress: Optional[float] = Field(None, ge=0.0, le=1.0, description="Parsing progress (0.0 to 1.0)")
...
class ExistingDocumentParseResponse(BaseModel):
    ...
    storage_path: str = Field(..., description="Original GCS storage path")
    parsed_storage_path: str = Field(..., description="GCS path where parsed content is stored")
    parsed_content: Optional[str] = Field(None, description="Parsed document content (if exists)")
    parsed_metadata: Optional[Dict[str, Any]] = Field(None, description="Metadata of parsed file")
...
class SaveParsedDocumentRequest(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Additional metadata for the parsed document"
    )
    ...
    def validate_target_path(cls, v: str) -> str:
        ...
        parts = v.split('/')
        if len(parts) < 3:
            raise ValueError("Target path must have format: 'organization/parsed/folder'")
...
class DocumentIndexingInfo(BaseModel):
    ...
    pinecone_indexed: bool = Field(..., description="Whether document was indexed in Pinecone")
    bm25_indexed: bool = Field(..., description="Whether document was indexed in BM25")
    embedding_dimension: Optional[int] = Field(None, description="Dimension of generated embedding")
    token_count: Optional[int] = Field(None, description="Number of tokens for BM25 indexing")
...
class SaveParsedDocumentResponse(BaseModel):
    ...
    gcs_path: str = Field(..., description="Full GCS storage path where content was saved")
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When the file was saved")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
    indexing: DocumentIndexingInfo = Field(..., description="Document indexing information")
...
class DocumentContentUpdateRequest(BaseModel):
    ...
    sync_to_gcs: bool = Field(
        default=True,
        description="Whether to sync the content to GCS storage"
    )
    ...
    def validate_content(cls, v: str) -> str:
        ...
        if len(v.encode('utf-8')) > max_content_size:
            raise ValueError(f"Content exceeds maximum size of {max_content_size // 1024}KB")
        return v
...
class DocumentContentUpdateResponse(BaseModel):
    ...
    gcs_path: Optional[str] = Field(None, description="GCS path where content was saved")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When the update occurred")
...
class DocumentSummaryMetadata(BaseModel):
    ...
    generated_at: str = Field(..., description="When summary was generated (ISO format)")
    model: str = Field(default="gpt-4o-mini", description="AI model used for summarization")
    ...
    has_custom_prompt: bool = Field(default=False, description="Whether custom prompt was used")
    update_type: Optional[str] = Field(None, description="Type of update (generated, direct_update, regenerated)")
...
class DocumentSummarizeResponse(BaseModel):
    ...
    summary_metadata: Dict[str, Any] = Field(..., description="Summary generation metadata")
    timestamp: str = Field(..., description="Response generation timestamp (ISO format)")
...
class DocumentSummaryUpdateRequest(BaseModel):
    ...
    prompt: Optional[str] = Field(None, description="Custom prompt to regenerate summary")
...
class DocumentAIContentRequest(BaseModel):
    ...
    faq: Optional[List[Dict[str, str]]] = Field(None, description="AI-generated FAQ items")
    questions: Optional[List[str]] = Field(None, description="AI-generated questions")
    
    @field_validator('faq')
    @classmethod
    def validate_faq_structure(cls, v: Optional[List[Dict[str, str]]]) -> Optional[List[Dict[str, str]]]:
        ...
        for i, item in enumerate(v):
            ...
            if 'question' not in item or 'answer' not in item:
                raise ValueError(f"FAQ item {i} must contain 'question' and 'answer' keys")
...
class DocumentAIContentResponse(BaseModel):
    ...
    summary: Optional[str] = Field(None, description="AI-generated document summary")
    faq: List[Dict[str, str]] = Field(default_factory=list, description="AI-generated FAQ items")
    questions: List[str] = Field(default_factory=list, description="AI-generated questions")
...
class DocumentSummaryResponse(BaseModel):
    ...
    summary_metadata: Dict[str, Any] = Field(default_factory=dict, description="Summary generation metadata")
    has_summary: bool = Field(..., description="Whether document has a summary available")
    summary_preview: str = Field(default="", description="Preview of summary (first 150 characters)")
    updated_at: Optional[str] = Field(None, description="When summary was last updated (ISO format)")
...
class DocumentFAQRequest(BaseModel):
    ...
    faq_count: int = Field(default=5, ge=1, le=10, description="Number of FAQs to generate (1-10)")
...
class DocumentFAQResponse(BaseModel):
    ...
    document_id: str = Field(..., description="Document ID that FAQs were generated for")
    filename: str = Field(..., description="Original document filename")
...
class DocumentFAQUpdateRequest(BaseModel):
    ...
    faq: Optional[List[Dict[str, str]]] = Field(None, description="Direct FAQ content to set")
    prompt: Optional[str] = Field(None, description="Custom prompt to regenerate FAQ")
    faq_count: Optional[int] = Field(None, ge=1, le=10, description="Number of FAQs to generate when using prompt (1-10)")
...
class DocumentFAQRetrievalResponse(BaseModel):
    ...
    document_id: str = Field(..., description="Document unique identifier")
    filename: str = Field(..., description="Document filename")
    ai_faq: Optional[List[Dict[str, str]]] = Field(None, description="AI-generated FAQ items")
    faq_metadata: Dict[str, Any] = Field(default_factory=dict, description="FAQ generation metadata")
    ...
    faq_preview: str = Field(default="", description="Preview of FAQ (first question and answer)")
    updated_at: Optional[str] = Field(None, description="When FAQ was last updated (ISO format)")
...
class DocumentQuestionsRequest(BaseModel):
    ...
    prompt: Optional[str] = Field(None, description="Custom prompt for questions generation")
    question_count: int = Field(default=5, ge=1, le=20, description="Number of questions to generate (1-20)")
...
class DocumentQuestionsResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether questions generation was successful")
    document_id: str = Field(..., description="Document ID that questions were generated for")
    filename: str = Field(..., description="Original document filename")
    ai_questions: List[str] = Field(..., description="Generated AI questions")
    questions_metadata: Dict[str, Any] = Field(..., description="Questions generation metadata")
    timestamp: str = Field(..., description="Response generation timestamp (ISO format)")
    source: Optional[str] = Field(None, description="Source of questions - 'existing' or 'generated'")
    update_type: Optional[str] = Field(None, description="Type of update performed - 'direct_update' or 'regenerated'")
...
class DocumentQuestionsUpdateRequest(BaseModel):
    ...
    questions: Optional[List[str]] = Field(None, description="Direct questions content to set")
    prompt: Optional[str] = Field(None, description="Custom prompt to regenerate questions")
    question_count: Optional[int] = Field(None, ge=1, le=20, description="Number of questions to generate when using prompt (1-20)")
    
    @field_validator('questions')
    @classmethod
    def validate_questions_structure(cls, v: Optional[List[str]]) -> Optional[List[str]]:
...
class DocumentQuestionsRetrievalResponse(BaseModel):
    ...
    document_id: str = Field(..., description="Document unique identifier")
    filename: str = Field(..., description="Document filename")
    ai_questions: Optional[List[str]] = Field(None, description="AI-generated questions")
    questions_metadata: Dict[str, Any] = Field(default_factory=dict, description="Questions generation metadata")
    has_questions: bool = Field(..., description="Whether document has questions available")
    questions_count: int = Field(default=0, description="Number of questions")
    questions_preview: str = Field(default="", description="Preview of questions (first 3 questions)")
    updated_at: Optional[str] = Field(None, description="When questions were last updated (ISO format)")
