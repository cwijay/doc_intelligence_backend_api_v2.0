...
class Folder(BaseModel):
    ...
    id: Optional[str] = Field(None, description="Unique folder identifier (Firestore document ID)")
    ...
    parent_folder_id: Optional[str] = Field(None, description="Parent folder ID (null for root folders)")
    path: str = Field(..., description="Full folder path (e.g., /root/folder1/subfolder)")
    
    # Metadata
    created_by: str = Field(..., description="User ID who created the folder")
    
    # Status
    is_active: bool = Field(default=True, description="Whether folder is active (for soft delete)")
    
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When folder was created")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="When folder was last updated")
...
def _find_node_in_tree(tree: Dict[str, Any], folder_id: str) -> Optional[Dict[str, Any]]:
    """
