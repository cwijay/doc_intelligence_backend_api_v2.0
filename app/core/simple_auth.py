...
class SimpleAuthManager:
    ...
    def create_session(
        ...
        with self._lock:
            ...
            logger.info("Session created with refresh token", 
    ...
    def refresh_session(self, refresh_token: str) -> Optional[SimpleSession]:
        ...
        with self._lock:
            if refresh_token not in self._refresh_tokens:
                logger.warning("Refresh token not found", refresh_token=refresh_token[:8] + "...")
                return None
            ...
            if old_session.is_refresh_expired():
                ...
                del self._refresh_tokens[refresh_token]
                del self._sessions[session_id]
                logger.info("Refresh token expired and cleaned up", 
                           refresh_token=refresh_token[:8] + "...",
    ...
    def validate_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "can_refresh": session.refresh_token is not None and not session.is_refresh_expired()
        }
...
def get_current_user_simple(
    ...
    with simple_auth_manager._lock:
        ...
        if session.is_expired():
            ...
            # Also remove refresh token mapping
            if session.refresh_token and session.refresh_token in simple_auth_manager._refresh_tokens:
            ...
            raise TokenExpiredError("Session token has expired", expires_at=session.expires_at.isoformat())
