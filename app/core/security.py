...
class EnterpriseTokenManager:
        ...
        with self._lock:
            try:
                ...
                if token_info.refresh_token_family_id:
                    if token_info.refresh_token_family_id not in self._refresh_token_families:
                        self._refresh_token_families[token_info.refresh_token_family_id] = set()
                    self._refresh_token_families[token_info.refresh_token_family_id].add(token_info.token_id)
                
                logger.debug("Token registered successfully", 
    ...
    def invalidate_user_tokens(self, user_id: str, org_id: str, exclude_token_id: Optional[str] = None) -> int:
        ...
        with self._lock:
            try:
                ...
                session.active_tokens = {exclude_token_id} if exclude_token_id else set()
                
                logger.info("Invalidated user tokens", 
                           user_id=user_id,
                           org_id=org_id,
                           count=len(tokens_to_invalidate),
                           excluded_token=exclude_token_id[:8] + "..." if exclude_token_id else None)
                
                return len(tokens_to_invalidate)
                
            except Exception as e:
                logger.error("Failed to invalidate user tokens", 
                           user_id=user_id, org_id=org_id, error=str(e))
                return 0
            ...
            try:
                ...
                for user_key, session in list(self._user_sessions.items()):
                    ...
                    session.active_tokens = {
                        tid for tid in session.active_tokens 
                        if tid in self._token_registry
                    }
                    
                    # Clean up old blacklisted tokens
                    expired_blacklisted = {
                        tid for tid in session.blacklisted_tokens
                    ...
                    if (not session.active_tokens and 
                        not session.blacklisted_tokens and 
                        (now - session.last_activity).total_seconds() > 86400):
...
def verify_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if token_id:
            ...
            with _enterprise_token_manager._lock:
                if token_id in _enterprise_token_manager._token_registry:
                    _enterprise_token_manager._token_registry[token_id].last_used = datetime.utcnow()
        ...
        logger.warning("Token verification failed: invalid signature - possible tampering attempt")
        return None
...
def blacklist_token(token: str, reason: str = "logout") -> bool:
    ...
    try:
        ...
        payload = jwt.decode(token, settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM], verify=False)
        token_id = payload.get("jti")
...
def verify_token_detailed(token: str) -> tuple[Optional[Dict[str, Any]], TokenValidationResult]:
    """
...
def create_user_token_data(user_id: str, org_id: str, email: str, role: str) -> Dict[str, Any]:
    """
...
def generate_invitation_token(org_id: str, email: str, role: str, expires_hours: int = 168) -> str:
    """
...
def verify_invitation_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if payload.get("type") != "invitation":
            ...
            return None
        logger.debug("Invitation token verified successfully", 
        ...
        logger.error("Invitation token verification failed: unexpected JWT error", error=str(e))
        return None
...
def invalidate_user_sessions(user_id: str, org_id: str, exclude_token_id: Optional[str] = None) -> int:
    """
    ...
    """
    return _enterprise_token_manager.invalidate_user_tokens(user_id, org_id, exclude_token_id)
...
def get_current_user_org(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
    """
    ...
    if settings.ENABLE_AUTH_AUDIT_LOGGING:
        logger.info("🔐 ENTERPRISE AUTHENTICATION STARTED", 
    ...
    if settings.ENABLE_AUTH_AUDIT_LOGGING:
        logger.info("✅ ENTERPRISE AUTHENTICATION SUCCESS", 
                   org_id=org_id, 
                   user_id=user_id,
                   token_id=token_id[:8] + "..." if token_id else "legacy",
