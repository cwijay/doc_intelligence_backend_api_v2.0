...
class Settings(BaseSettings):
    ...
    JWT_SECRET_KEY: str = Field(..., description="Secret key for JWT tokens - must be cryptographically secure")
    ...
    INVALIDATE_TOKENS_ON_LOGIN: bool = Field(
        default=True, 
        description="Invalidate all previous tokens when user logs in"
    )
    ...
    TOKEN_BLACKLIST_CLEANUP_HOURS: int = Field(
        default=24, 
        description="Hours between automatic blacklist cleanup runs"
    )
    ...
    REFRESH_SESSION_DURATION_DAYS: int = Field(
        default=7, 
        description="Refresh token duration in days (recommended: 7-30 days)"
    )
    
    TOKEN_GRACE_PERIOD_MINUTES: int = Field(
        default=10, 
        description="Grace period for token expiration in minutes"
    )
    
    # Security Monitoring
    ENABLE_AUTH_AUDIT_LOGGING: bool = Field(
        default=True, 
        description="Enable detailed authentication audit logging"
    )
    ...
    FIREBASE_DATABASE_ID: Optional[str] = None  # Firestore database ID (default: "(default)")
    FIREBASE_SERVICE_ACCOUNT_JSON: Optional[str] = None  # JSON string of service account
    ...
    def resolved_cors_origins(self) -> List[str]:
        ...
        if self.is_production:
                ...
                try:
                    ...
                    if self.PRODUCTION_CORS_ORIGINS.startswith('['):
                        ...
                        if ';' in self.PRODUCTION_CORS_ORIGINS:
                            prod_origins = [origin.strip() for origin in self.PRODUCTION_CORS_ORIGINS.split(';')]
                        else:
                            prod_origins = [origin.strip() for origin in self.PRODUCTION_CORS_ORIGINS.split(',')]
            ...
            if service_name and revision:
                ...
                if project_id:
                    cloud_run_url = f"https://{service_name}-{revision[:8]}-{region}.a.run.app"
            ...
            if self.FRONTEND_SERVICE_NAME:
                ...
                if project_id:
                    ...
                    frontend_pattern_url = f"https://{self.FRONTEND_SERVICE_NAME}-*-{region}.a.run.app"
                    ...
                    if hasattr(self, '_log_frontend_pattern'):
                        import logging
                        logging.getLogger(__name__).info(f"Frontend pattern would be: {frontend_pattern_url}")
                    ...
                    if self.FRONTEND_CLOUD_RUN_URL and self.FRONTEND_SERVICE_NAME in self.FRONTEND_CLOUD_RUN_URL:
