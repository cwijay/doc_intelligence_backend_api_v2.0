...
from app.models.schemas import (
    DocumentResponse, DocumentList, DocumentUploadResponse,
    DocumentDownloadResponse, DocumentFilters, PaginationParams
)
...
class DocumentService(DocumentBaseService):
    ...
    async def _check_storage_path_exists(self, org_id: str, storage_path: str) -> bool:
        ...
        return await self.storage_service._check_storage_path_exists(org_id, storage_path)
    
    def _generate_unique_storage_path(self, base_storage_path: str, filename: str) -> str:
        """Delegate to storage service."""
        return self.storage_service._generate_unique_storage_path(base_storage_path, filename)
    
    async def _ensure_unique_storage_path(self, org_id: str, storage_path: str, filename: str) -> str:
        """Delegate to storage service."""
        return await self.storage_service._ensure_unique_storage_path(org_id, storage_path, filename)
    ...
    async def validate_content_sync(
        ...
        """Delegate to storage service."""
        return await self.storage_service.validate_content_sync(org_id, document_id, storage_path)
    ...
    async def delete_document(self, org_id: str, document_id: str) -> Dict[str, Any]:
        """Delegate to CRUD service."""
        return await self.crud_service.delete_document(org_id=org_id, document_id=document_id)
