        ...
        try:
            ...
            async for doc in docs:
                ...
                if filename in storage_path:
                    self.logger.info(
                        "Found document by storage_path match",
                        org_id=org_id,
                        document_id=doc.id,
                        filename=filename,
...
Please provide a comprehensive markdown summary following the structure outlined in the system prompt."""
    ...
    async def _save_summary_to_gcs(
        self, org_id: str, original_storage_path: str, summary: str
    ) -> str:
        """
        ...
        try:
            ...
            if len(path_parts) < 3:
                raise SummarizationError(
                    f"Invalid storage path format: {original_storage_path}"
                )
            ...
            summary_storage_path = (
                f"{org_name}/summaries/{folder_name}/{summary_filename}"
            )
            ...
            self.logger.error(
                "Unexpected error saving summary",
                org_id=org_id,
                original_storage_path=original_storage_path,
                error=str(e),
            )
    ...
    async def _update_document_with_summary(
        ...
        try:
            ...
            self.logger.info(
                "Updated document with summary information",
                org_id=org_id,
            ...
            self.logger.error(
                "Failed to update document with summary",
                org_id=org_id,
                document_id=document_id,
                error=str(e),
            )
    ...
    async def summarize_document_content(
        ...
        try:
            self.logger.info(
                "Starting document summarization", org_id=org_id, filename=filename
            )

            # Step 1: Find document in Firestore
            document_id, document_data = await self._find_document_by_filename(
                org_id, filename
            )
