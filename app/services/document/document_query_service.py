...
from app.models.schemas import (
    DocumentList, DocumentResponse, DocumentFilters, PaginationParams
)
...
class DocumentQueryService(DocumentBaseService):
    ...
    async def list_documents(
        ...
        try:
            ...
            if filters and filters.folder_path and "/original/" in filters.folder_path:
                ...
                return await self._list_documents_from_gcs(org_id, filters.folder_path, pagination, filters, storage_service, validation_service)
            ...
            async for doc in all_docs:
                try:
                    ...
                    if filters and filters.folder_path:
                        document_folder = validation_service._extract_folder_from_storage_path(document.storage_path)
    ...
    async def _list_documents_from_gcs(
        ...
        """
        try:
            self.logger.info("Listing documents from GCS", 
                           org_id=org_id,
            ...
            try:
                ...
                self.logger.info("Retrieved GCS file list", 
                               org_id=org_id,
                               folder_path=folder_path,
                               file_count=len(gcs_files))
            except Exception as e:
                self.logger.error("Failed to list files from GCS", 
                                org_id=org_id,
                                folder_path=folder_path,
                                error=str(e))
            ...
            for gcs_file in gcs_files:
                try:
                    ...
                    self.logger.error("Failed to process GCS file", 
                                    gcs_file=gcs_file,
                                    error=str(e))
                    continue
            ...
            for doc in paginated_documents:
                try:
                    ...
                    self.logger.error("Failed to convert GCS Document to DocumentResponse", 
                                    document_id=doc.id,
                                    error=str(e))
                    continue
            ...
            self.logger.info("GCS document listing completed", 
                           org_id=org_id,
                           folder_path=folder_path,
                           total_files=total,
        ...
        try:
            ...
            async for doc in docs:
                try:
                    ...
                    self.logger.warning("Error processing document in folder query", 
                                      document_id=doc.id, 
                                      error=str(e))
                    continue
