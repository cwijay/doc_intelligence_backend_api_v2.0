    ...
    async def sync_content_to_firestore(
        ...
        try:
            if not firebase_manager.is_initialized:
                ...
                return {"success": False, "error": error_msg, "document_updated": False}
        ...
        try:
            ...
            if not gcs_storage_path:
                ...
                if not gcs_storage_path:
                    ...
                    if original_path:
                        if storage_service:
                            gcs_storage_path = (
                                storage_service._generate_parsed_storage_path(
                                    original_path
                                )
                            )
                        else:
            ...
            while retry_count < self.max_sync_retries:
                try:
                    ...
                    last_error = e
                    self.logger.warning(
                        "GCS sync attempt failed, retrying",
                        org_id=org_id,
                        document_id=document_id,
                        gcs_storage_path=gcs_storage_path,
                        retry_attempt=retry_count,
            ...
            self.logger.error(
                "Failed to sync content to GCS after retries",
                org_id=org_id,
                document_id=document_id,
                gcs_storage_path=gcs_storage_path,
                retry_attempts=retry_count,
        ...
        try:
            ...
            async for doc in docs:
                try:
                    ...
                    if doc_sync_result.get("valid") and doc_sync_result.get(
                        "synchronized"
                    ):
                        validation_results["documents_synchronized"] += 1
                    elif doc_sync_result.get("valid") and not doc_sync_result.get(
                        "synchronized"
                    ):
                        ...
                        validation_results["sync_issues"].append(
                            {
                                ...
                                "description": doc_sync_result.get(
                                    "note", "Content differs between stores"
                                ),
                                "firestore_content_length": doc_sync_result.get(
                                    "firestore_content_length", 0
                                ),
                                "gcs_content_length": doc_sync_result.get(
                                    "gcs_content_length", 0
                                ),
                            }
                        )
                    else:
                    ...
                    self.logger.error(
                        "Error validating document sync",
                        org_id=org_id,
                        document_id=getattr(doc, "id", "unknown"),
                        error=str(e),
                    )
            ...
            if (
                validation_results["documents_synchronized"]
                == validation_results["documents_with_content"]
            ):
