...
class DocumentAIService(DocumentBaseService):
        ...
        try:
            ...
            async for doc in docs:
                ...
                if document.is_active:
                    ...
                    self.logger.debug(
                        "Document AI fields state",
                        document_id=doc.id,
                        has_ai_summary=document.has_ai_summary,
                        ai_summary_length=(
                            len(document.ai_summary) if document.ai_summary else 0
                        ),
                        ...
                        questions_count=(
                            len(document.ai_questions) if document.ai_questions else 0
                        ),
                    )
            ...
            raise DocumentNotFoundError(
                f"Document with filename '{filename}' not found in organization {org_id}"
            )

        except Exception as e:
            self.logger.error(
                "Error searching for document",
                org_id=org_id,
                filename=filename,
                error=str(e),
            )
    ...
    async def _generate_summary(
        self, content: str, document: Document, custom_prompt: Optional[str] = None
    ) -> str:
        """
...
Return exactly {faq_count} FAQ items as a valid JSON array."""
            ...
            if len(faq_data) != faq_count:
                self.logger.warning(
                    "FAQ count mismatch", expected=faq_count, received=len(faq_data)
                )
            ...
            self.logger.error(
                "Failed to parse FAQ response", filename=filename, error=str(e)
            )
...
Return exactly {question_count} questions as a valid JSON array of strings."""
            ...
            if not response or not response.content:
                self.logger.error(
                    "LangChain returned empty response",
                    response=str(response),
                    has_content=bool(
                        response and response.content if response else False
                    ),
                )
                raise QuestionsGenerationError("LangChain returned empty response")

            # Log the raw response for debugging
            self.logger.debug("Raw LangChain response", content=response.content[:500])
            ...
            self.logger.error(
                "Failed to parse questions response", filename=filename, error=str(e)
            )
    ...
    async def generate_summary_by_filename(
        self, org_id: str, filename: str, custom_prompt: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            document_id, document = await self._find_document_by_filename(
                org_id, filename
            )
            ...
            if document.has_ai_summary:
                self.logger.info(
                    "Summary already exists for document, returning existing summary",
                    org_id=org_id,
                    filename=filename,
            ...
            self.logger.debug(
                "Summary - About to save to Firestore",
                org_id=org_id,
                filename=filename,
                document_id=document_id,
            ...
            self.logger.info(
                "Summary - Successfully saved to Firestore",
                org_id=org_id,
                filename=filename,
                document_id=document_id,
                summary_length=len(summary),
        ...
        try:
            if not summary and not custom_prompt:
                raise DocumentValidationError(
                    "Either summary or custom_prompt must be provided"
                )
            ...
            if summary:
                ...
                doc_dict = document.to_dict()
                self.logger.debug(
                    "Summary Update - About to save to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    has_ai_summary_in_dict=bool(doc_dict.get("ai_summary")),
                ...
                self.logger.info(
                    "Summary Update - Successfully saved to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    summary_length=len(summary),
                ...
                response = await self.generate_summary_by_filename(
                    org_id, filename, custom_prompt
                )
    ...
    async def generate_faq_by_filename(
        ...
        try:
            ...
            if document.has_ai_faq:
                self.logger.info(
                    "FAQ already exists for document, returning existing FAQ",
                    org_id=org_id,
                    filename=filename,
            ...
            faq_items = await self._generate_faq(
                content, document, faq_count, custom_prompt
            )
            ...
            doc_dict = document.to_dict()
            self.logger.debug(
                "FAQ - About to save to Firestore",
                org_id=org_id,
                filename=filename,
                document_id=document_id,
            ...
            self.logger.info(
                "FAQ - Successfully saved to Firestore",
                org_id=org_id,
                filename=filename,
                document_id=document_id,
                faq_count=len(faq_items),
    ...
    async def get_faq_by_filename(self, org_id: str, filename: str) -> Dict[str, Any]:
        """
    ...
    async def update_faq_by_filename(
        ...
        try:
            if not faq and not custom_prompt:
                raise DocumentValidationError(
                    "Either faq or custom_prompt must be provided"
                )
            ...
            if faq:
                ...
                doc_dict = document.to_dict()
                self.logger.debug(
                    "FAQ Update - About to save to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    has_ai_faq_in_dict=bool(doc_dict.get("ai_faq")),
                ...
                self.logger.info(
                    "FAQ Update - Successfully saved to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    faq_count=len(faq),
                ...
                response = await self.generate_faq_by_filename(
                    org_id, filename, faq_count or 5, custom_prompt
                )
                response["update_type"] = "regenerated"

            self.logger.info(
                "Document FAQ updated",
                org_id=org_id,
                filename=filename,
    ...
    async def generate_questions_by_filename(
        ...
        try:
            ...
            if document.has_ai_questions:
                self.logger.info(
                    "Questions already exist for document, returning existing questions",
                    org_id=org_id,
                    filename=filename,
            ...
            questions = await self._generate_questions(
                content, document, question_count, custom_prompt
            )
            ...
            doc_dict = document.to_dict()
            self.logger.debug(
                "Questions - About to save to Firestore",
                org_id=org_id,
                filename=filename,
                document_id=document_id,
            ...
            self.logger.info(
                "Questions - Successfully saved to Firestore",
                org_id=org_id,
                filename=filename,
                document_id=document_id,
                question_count=len(questions),
            ...
            raise QuestionsGenerationError(
                f"Unexpected error during questions generation: {e}"
            )
    ...
    async def get_questions_by_filename(
        ...
        try:
            ...
            self.logger.debug(
                "Retrieved questions for document",
                org_id=org_id,
                filename=filename,
                has_questions=document.has_ai_questions,
    ...
    async def update_questions_by_filename(
        ...
        try:
            if questions is None and not custom_prompt:
                raise DocumentValidationError(
                    "Either questions or custom_prompt must be provided"
                )
            ...
            if questions is not None:
                ...
                doc_dict = document.to_dict()
                self.logger.debug(
                    "Questions Update - About to save to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    has_ai_questions_in_dict=bool(doc_dict.get("ai_questions")),
                ...
                self.logger.info(
                    "Questions Update - Successfully saved to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    question_count=len(questions),
                ...
                questions = await self._generate_questions(
                    content, document, question_count or 5, custom_prompt
                )
                ...
                doc_dict = document.to_dict()
                self.logger.debug(
                    "Questions Regenerate - About to save to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    has_ai_questions_in_dict=bool(doc_dict.get("ai_questions")),
                ...
                self.logger.info(
                    "Questions Regenerate - Successfully saved to Firestore",
                    org_id=org_id,
                    filename=filename,
                    document_id=document_id,
                    question_count=len(questions),
            ...
            self.logger.error(
                "Error updating document questions",
                org_id=org_id,
                filename=filename,
                error=str(e),
            )
