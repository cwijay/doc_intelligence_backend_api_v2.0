...
class DocumentParsingService:
    ...
    async def parse_document_from_gcs(
        ...
        try:
            ...
            self.logger.debug("Cleaned storage path", 
            ...
            try:
                ...
                self.logger.debug("Retrieved GCS metadata", 
                                storage_path=cleaned_storage_path,
                                file_size=gcs_metadata.get("size"))
            except GCSObjectNotFoundError:
                raise DocumentNotFoundError(f"Document not found in GCS: {cleaned_storage_path}")
            ...
            try:
                ...
                self.logger.debug("Downloaded file from GCS", 
                                storage_path=storage_path,
            ...
            try:
                ...
                self.logger.info("Starting document parsing with LlamaParse", 
                ...
                parsed_storage_path = self._generate_parsed_storage_path(cleaned_storage_path)
                ...
                try:
                    ...
                    self.logger.info("Stored parsed content in GCS", 
                                   original_path=cleaned_storage_path,
                                   parsed_path=parsed_storage_path)
                except GCSClientError as e:
                    self.logger.warning("Failed to store parsed content in GCS", 
                                      error=str(e),
                                      parsed_path=parsed_storage_path)
                ...
                try:
                    ...
                    self.logger.warning("Failed to update document status in Firestore", 
                                      error=str(e),
                                      storage_path=cleaned_storage_path)
                ...
                self.logger.info("Document parsing workflow completed successfully", 
                               org_id=org_id,
                               storage_path=cleaned_storage_path,
                ...
                try:
                    ...
                    self.logger.debug("Cleaned up temporary file", temp_file_path=temp_file_path)
                except OSError as e:
                    self.logger.warning("Failed to cleanup temporary file", 
                                      temp_file_path=temp_file_path,
                                      error=str(e))
        
        except Exception as e:
            self.logger.error("Document parsing failed", 
                            org_id=org_id,
                            storage_path=storage_path,
                            error=str(e),
    ...
    def _clean_storage_path(self, storage_path: str) -> str:
        ...
        if bucket_name and storage_path.startswith(f"{bucket_name}/"):
            ...
            self.logger.debug("Removed bucket name prefix from storage path",
    ...
    async def _update_document_status(
        ...
        try:
            ...
            collection = get_collection("organizations").document(org_id).collection("documents")
            ...
            async for doc in docs:
                ...
                self.logger.info("Updated document status in Firestore", 
                               org_id=org_id,
                               document_id=doc.id,
                               status=status.value,
                               storage_path=storage_path)
            ...
            if not document_found:
                self.logger.debug("No document found in Firestore for storage path", 
                                org_id=org_id,
                                storage_path=storage_path)
                
        except GoogleAPIError as e:
            self.logger.error("Failed to update document status in Firestore", 
                            org_id=org_id,
                            storage_path=storage_path,
                            error=str(e))
            raise
    ...
    async def get_parsed_document(
        ...
        try:
            ...
            try:
                ...
                parsed_content_bytes = gcs_client.download_document_file(parsed_storage_path)
                parsed_content = parsed_content_bytes.decode('utf-8')
                
                self.logger.info("Retrieved existing parsed document", 
                               org_id=org_id,
                               original_path=cleaned_storage_path,
                               parsed_path=parsed_storage_path)
            ...
            self.logger.error("Error checking for parsed document", 
                            org_id=org_id,
                            storage_path=storage_path,
                            error=str(e))
    ...
    async def save_parsed_content_to_gcs(
        ...
        try:
            ...
            self.logger.info("Starting save parsed content to GCS", 
            ...
            self.logger.debug("Constructed storage path", 
                            target_path=target_path,
                            cleaned_target_path=cleaned_target_path,
                            original_filename=original_filename,
            ...
            try:
                ...
                file_exists = True
                self.logger.debug("File already exists in GCS", storage_path=storage_path)
            except GCSObjectNotFoundError:
                file_exists = False
                self.logger.debug("File does not exist in GCS, will create new", storage_path=storage_path)
            ...
            try:
                ...
                self.logger.info("Successfully saved parsed content to GCS", 
                               storage_path=storage_path,
            ...
            try:
                if firebase_manager.is_initialized:
                    ...
                    collection = get_collection("organizations").document(org_id).collection("documents")
                    query = collection.where(filter=FieldFilter("filename", "==", original_filename))
                    ...
                    async for doc in docs:
                        ...
                        self.logger.info("Updated document in Firestore with parsed content", 
                                       org_id=org_id,
                                       document_id=doc.id,
                    ...
                    if not document_found:
                        self.logger.warning("No document found in Firestore for filename", 
                                          org_id=org_id,
                                          filename=original_filename)
                else:
                    self.logger.warning("Firebase not initialized, skipping Firestore update")
                    
            except GoogleAPIError as e:
                self.logger.error("Failed to update document in Firestore", 
                                org_id=org_id,
                                filename=original_filename,
                                error=str(e))
            ...
            self.logger.info("Save parsed content operation completed successfully", 
                           **response)
            
            return response
            
        except Exception as e:
            self.logger.error("Error saving parsed content", 
                            target_path=target_path,
                            original_filename=original_filename,
                            org_id=org_id,
                            error=str(e))
