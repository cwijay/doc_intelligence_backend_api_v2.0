...
class FolderService:
    ...
    async def _get_organization_name(self, org_id: str) -> str:
        ...
        try:
            ...
            self.logger.error("Failed to get organization name", 
                            org_id=org_id, 
                            error=str(e))
            raise FolderValidationError(f"Could not fetch organization name for ID {org_id}: {e}")
    ...
    async def create_folder(
        ...
        try:
            ...
            if folder_data.parent_folder_id:
                parent_folder = await self._get_folder_by_id(org_id, folder_data.parent_folder_id)
                if not parent_folder:
                    raise FolderValidationError(f"Parent folder {folder_data.parent_folder_id} not found")
                
                parent_path = parent_folder.path
                
                # Check depth limit
                if parent_folder.depth >= self.max_depth:
                    raise FolderValidationError(f"Maximum folder depth ({self.max_depth}) exceeded")
            ...
            if gcs_client.is_initialized:
                ...
                try:
                    existing_files = gcs_client.list_folder_contents(org_name, folder_path, "original")
                    ...
                    self.logger.debug("Could not check for existing folder in GCS", 
                                    org_id=org_id, 
            ...
            try:
                ...
                self.logger.info("Created GCS folder structure", 
                               org_id=org_id,
                               org_name=org_name, 
                               folder_path=folder_path,
                               gcs_result=gcs_result)
            except GCSClientError as e:
                self.logger.error("Failed to create GCS folder structure", 
                                org_id=org_id, 
                                folder_path=folder_path,
                                error=str(e))
            ...
            self.logger.info("Folder created in GCS only", 
                           org_id=org_id, 
                           org_name=org_name,
                           name=folder.name,
                           path=folder.path)
            ...
            self.logger.error("Error creating folder", 
                            org_id=org_id, 
                            error=str(e))
            raise
    ...
    async def get_folder(self, org_id: str, folder_id: str) -> FolderResponse:
        ...
        try:
            ...
            self.logger.debug("Folder retrieved from GCS", org_id=org_id, folder_path=folder_path)
            ...
            self.logger.error("Error retrieving folder from GCS", 
                            org_id=org_id, 
                            folder_id=folder_id, 
                            error=str(e))
            raise
    ...
    async def list_folders(
        ...
        try:
            ...
            try:
                ...
                for blob in parsed_blobs:
                    if blob.name.endswith('.folder_placeholder'):
                        ...
                        parts = blob.name.split('/')
                        if len(parts) >= 3:
                            folder_path = '/'.join(parts[2:-1])  # Remove org/type and placeholder
                            if folder_path:  # Skip empty paths
                                folder_paths.add(folder_path)
                ...
                self.logger.error("Error listing GCS folders", 
                                org_id=org_id, 
                                org_name=org_name,
                                error=str(e))
            ...
            folder_responses = [FolderResponse.model_validate(folder) for folder in paginated_folders]

            # Calculate pagination info
            total_pages = math.ceil(total / pagination.per_page) if total > 0 else 0

            self.logger.debug("GCS folders listed", 
                            org_id=org_id,
                            org_name=org_name,
            ...
            self.logger.error("Error listing folders from GCS", org_id=org_id, error=str(e))
            raise
    ...
    async def get_folder_tree(self, org_id: str) -> FolderTree:
        ...
        try:
            ...
            self.logger.debug("Folder tree retrieved", 
                            org_id=org_id,
            ...
            self.logger.error("Error retrieving folder tree", org_id=org_id, error=str(e))
            raise
    ...
    async def move_folder(
        ...
        try:
            ...
            if new_parent_folder_id:
                new_parent_folder = await self._get_folder_by_id(org_id, new_parent_folder_id)
                if not new_parent_folder or not new_parent_folder.is_active:
                    raise FolderValidationError(f"New parent folder {new_parent_folder_id} not found")
                ...
                if new_parent_path.startswith(folder.path):
                    raise FolderValidationError("Cannot move folder to its own descendant")
                
                # Check depth limit
                new_depth = new_parent_folder.depth + 1
                if new_depth > self.max_depth:
                    raise FolderValidationError(f"Move would exceed maximum depth ({self.max_depth})")
            ...
            if gcs_client.is_initialized:
                try:
                    ...
                    gcs_result = gcs_client.move_folder_structure(org_name, old_gcs_path, new_gcs_path)
                    self.logger.info("Moved GCS folder structure", 
                                   org_id=org_id,
                                   org_name=org_name, 
                                   folder_id=folder_id,
                    ...
                    self.logger.warning("Failed to move GCS folder structure", 
                                      org_id=org_id, 
                                      folder_id=folder_id,
                                      error=str(e))
                except FolderValidationError as e:
                    self.logger.warning("Failed to get organization name for GCS move", 
                                      org_id=org_id, 
                                      folder_id=folder_id,
                                      error=str(e))
            else:
                self.logger.debug("GCS not configured, skipping folder structure move", 
                                org_id=org_id, 
                                folder_id=folder_id)
            ...
            self.logger.error("Error moving folder", 
                            org_id=org_id, 
                            folder_id=folder_id, 
                            error=str(e))
            raise
    ...
    async def delete_folder(self, org_id: str, folder_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.logger.info("Deleted GCS folder structure", 
                               org_id=org_id,
                               org_name=org_name, 
                               folder_path=folder_path,
                               gcs_result=gcs_result)
                ...
                self.logger.error("Failed to delete GCS folder structure", 
                                org_id=org_id, 
                                folder_path=folder_path,
                                error=str(e))
                raise FolderValidationError(f"Failed to delete folder from GCS: {e}")

            self.logger.info("Folder deleted from GCS", 
                           org_id=org_id, 
                           org_name=org_name,
                           folder_path=folder_path)
            ...
            self.logger.error("Error deleting folder from GCS", 
                            org_id=org_id, 
                            folder_id=folder_id, 
                            error=str(e))
            raise
    ...
    async def get_folder_path(self, org_id: str, folder_id: str) -> str:
        ...
        try:
            ...
            self.logger.error("Error getting folder path", 
                            org_id=org_id, 
                            folder_id=folder_id, 
                            error=str(e))
            raise
    ...
    async def _get_folder_by_id(self, org_id: str, folder_id: str) -> Optional[Folder]:
        ...
        try:
            ...
            self.logger.error("Error getting folder by ID", 
                            org_id=org_id, 
                            folder_id=folder_id, 
                            error=str(e))
            return None
    ...
    async def _get_folder_by_name_and_parent(
        ...
        try:
            ...
            query = query.where(filter=FieldFilter("parent_folder_id", "==", parent_folder_id))
            ...
            self.logger.error("Error getting folder by name and parent", 
                            org_id=org_id, 
                            name=name, 
    ...
    async def _get_descendant_folders(self, org_id: str, folder_path: str) -> List[Folder]:
        ...
        try:
            ...
            self.logger.error("Error getting descendant folders", 
                            org_id=org_id, 
                            folder_path=folder_path,
                            error=str(e))
            return []
    ...
    async def _update_descendant_paths(
        ...
        try:
            ...
            for descendant in descendant_folders:
                ...
                relative_path = descendant.path[len(old_path):]  # Remove old parent path
                ...
                self.logger.debug("Updated descendant folder path", 
                                org_id=org_id,
                                folder_id=descendant.id,
                                old_path=descendant.path,
                                new_path=descendant_new_path)
        except Exception as e:
            self.logger.error("Error updating descendant paths", 
                            org_id=org_id,
    ...
    def _build_folder_tree(self, folders: List[Folder]) -> List[FolderWithChildren]:
        ...
        folder_responses = {
            ...
            for folder in folders if folder.id
        }
        ...
        for folder in folders:
            if folder.is_root:
                ...
                if parent_id in folder_responses:
                    folder_responses[parent_id].children.append(folder_responses[folder.id])
