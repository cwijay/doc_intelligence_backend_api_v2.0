...
class UserService:
    ...
    async def _verify_organization_exists(self, org_id: str) -> bool:
        ...
        try:
            ...
            if not firebase_manager.is_initialized:
                self.logger.warning("Firebase not initialized when verifying organization", org_id=org_id)
                return False
            ...
            self.logger.debug("Organization verification result", org_id=org_id, is_active=is_active)
            return is_active
        except Exception as e:
            self.logger.error("Error verifying organization", org_id=org_id, error=str(e), exc_info=True)
            return False

    async def _check_email_unique_in_org(self, org_id: str, email: str, exclude_user_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            self.logger.error("Error checking email uniqueness", org_id=org_id, email=email, error=str(e))
            return False

    async def _check_username_unique_in_org(self, org_id: str, username: str, exclude_user_id: Optional[str] = None) -> bool:
        """Check if username is unique within the organization."""
        try:
            collection = self._get_users_collection(org_id)
            query = collection.where(filter=FieldFilter("username", "==", username.lower()))
            ...
            self.logger.error("Error checking username uniqueness", org_id=org_id, username=username, error=str(e))
            return False
    ...
    async def create_user(
        ...
        try:
            ...
            if not await self._check_email_unique_in_org(org_id, user_data.email):
                raise UserAlreadyExistsError(f"User with email '{user_data.email}' already exists in organization")

            # Check username uniqueness within organization
            if not await self._check_username_unique_in_org(org_id, user_data.username):
                raise UserAlreadyExistsError(f"User with username '{user_data.username}' already exists in organization")
            ...
            self.logger.info("User created", org_id=org_id, user_id=user_id, email=user.email)
    ...
    async def get_user(
        ...
        try:
            ...
            if not doc.exists:
                raise UserNotFoundError(f"User with ID {user_id} not found in organization {org_id}")
            ...
            self.logger.error("Error retrieving user", org_id=org_id, user_id=user_id, error=str(e))
            raise
    ...
    async def update_user(
        ...
        try:
            ...
            if update_data.email and update_data.email.lower() != user.email:
                if not await self._check_email_unique_in_org(org_id, update_data.email, user_id):
                    raise UserAlreadyExistsError(f"User with email '{update_data.email}' already exists in organization")

            # Check username uniqueness if username is being updated
            if update_data.username and update_data.username.lower() != user.username:
                if not await self._check_username_unique_in_org(org_id, update_data.username, user_id):
                    raise UserAlreadyExistsError(f"User with username '{update_data.username}' already exists in organization")
            ...
            self.logger.info("User updated", org_id=org_id, user_id=user_id, updates=list(update_fields.keys()))
            ...
            self.logger.error("Error updating user", org_id=org_id, user_id=user_id, error=str(e))
            raise
    ...
    async def list_users(
        ...
        try:
            ...
            if filters:
                if filters.role:
                    firestore_filters.append(FieldFilter("role", "==", filters.role.value))
                
                if filters.is_active is not None:
                    # Update the active filter
                    firestore_filters[-1] = FieldFilter("is_active", "==", filters.is_active)
            ...
            async for doc in docs:
                ...
                if filters:
                    if filters.email and filters.email.lower() not in user.email.lower():
                        continue
                    if filters.username and filters.username.lower() not in user.username.lower():
                        continue
                    if filters.full_name and filters.full_name.lower() not in user.full_name.lower():
                        continue
            ...
            user_responses = [UserResponse.model_validate(user) for user in paginated_users]

            # Calculate pagination info
            total_pages = math.ceil(total / pagination.per_page) if total > 0 else 0

            self.logger.debug("Users listed", 
                            org_id=org_id,
    ...
    async def delete_user(
        ...
        try:
            ...
            self.logger.info("User deleted", org_id=org_id, user_id=user_id, email=user.email)
            ...
            self.logger.error("Error deleting user", org_id=org_id, user_id=user_id, error=str(e))
            raise
    ...
    async def verify_password(
        ...
        """
        try:
            self.logger.info("Verifying password", 
            ...
            self.logger.info("Password verification result", 
                           verified=result,
    ...
    async def get_user_by_email(
        ...
        try:
            ...
            self.logger.error("Error getting user by email", org_id=org_id, email=email, error=str(e))
            raise
    ...
    async def get_user_by_username(
        ...
        try:
            ...
            self.logger.error("Error getting user by username", org_id=org_id, username=username, error=str(e))
            raise
    ...
    async def _get_user_by_email_for_auth(
        ...
        try:
            ...
            self.logger.error("Error getting user by email for auth", org_id=org_id, email=email, error=str(e))
            return None
    ...
    async def _get_user_by_email_simple(self, email: str) -> Optional[User]:
        ...
        try:
            ...
            # Check if firebase is initialized
            if not firebase_manager.is_initialized:
                self.logger.error("Firebase not initialized for simple user lookup", email=email)
                return None
            
            self.logger.info("Firebase is initialized, proceeding with user lookup", email=email)
            
            # Get all organizations
            orgs_collection = get_collection("organizations")
            orgs_query = orgs_collection.where(filter=FieldFilter("is_active", "==", True))
            
            self.logger.info("Created organizations query, fetching active organizations", email=email)
            ...
            async for org_doc in orgs_docs:
                ...
                self.logger.info("Searching organization for user", 
                               org_id=org_id, 
                ...
                try:
                    ...
                    async for doc in docs:
                        ...
                        self.logger.info("Found user in organization", 
                                       email=email, 
                                       user_id=user.id, 
                                       org_id=org_id,
                        ...
                        if user.is_active:
                            self.logger.info("Returning active user", email=email, user_id=user.id, org_id=org_id)
                            return user
                        else:
                            self.logger.warning("User found but inactive", email=email, user_id=user.id, org_id=org_id)
                    
                    if users_found_in_org == 0:
                        self.logger.debug("No users found in organization", org_id=org_id, email=email)
                        
                except Exception as org_error:
                    self.logger.error("Error searching organization", 
                                    org_id=org_id, 
                                    email=email,
                                    error=str(org_error))
                    continue
            
            self.logger.warning("User not found in any organization", 
                              email=email, 
    ...
    async def _get_user_by_email_global_for_auth(
        self,
        email: str
    ) -> Optional[User]:
        """
        ...
        for attempt in range(max_retries):
            try:
                # Check if firebase is initialized
                if not firebase_manager.is_initialized:
                    self.logger.warning("Firebase not initialized for global auth user search", 
                                      email=email, attempt=attempt + 1)
                    if attempt < max_retries - 1:
                        await asyncio.sleep(retry_delay)
                        continue
                    return None
                    
                self.logger.debug("Starting global user search for auth", 
                                email=email, attempt=attempt + 1, max_retries=max_retries)
                ...
                try:
                    ...
                    async for org_doc in org_docs:
                        org_id = org_doc.id
                        organizations_searched += 1
                        self.logger.debug("Searching in organization for auth", 
                                        org_id=org_id, email=email, org_count=organizations_searched)
                        
                        try:
                            # Search for user in this organization using internal auth method
                            user = await self._get_user_by_email_for_auth(org_id, email)
                            if user:
                                self.logger.info("Global auth user search found user", 
                                                email=email, 
                                                user_id=user.id, 
                                                org_id=org_id,
                                                organizations_searched=organizations_searched,
                            ...
                            self.logger.debug("Error searching in specific org for auth", 
                                            org_id=org_id, 
                                            email=email, 
                    ...
                    self.logger.info("Global auth user search completed - no user found", 
                                   email=email, 
                                   organizations_searched=organizations_searched,
                                   attempt=attempt + 1)
                    return None
                    ...
                    self.logger.warning("Error executing organizations query", 
                                      email=email,
                ...
                self.logger.error("Error in global auth user search", 
                                email=email, 
                                error=str(e),
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.info("Retrying global user search", 
                                   email=email, 
                                   attempt=attempt + 1, 
                                   wait_time=wait_time)
                    ...
                    self.logger.error("Global auth user search failed after all retries", 
                                    email=email, attempts=max_retries)
                    return None
    ...
    async def get_user_by_email_global(
        ...
        try:
            # Check if firebase is initialized
            if not firebase_manager.is_initialized:
                self.logger.warning("Firebase not initialized for global user search", email=email)
                return None
            ...
            async for org_doc in org_docs:
                org_id = org_doc.id
                self.logger.debug("Searching in organization", org_id=org_id, email=email)
                
                try:
                    # Search for user in this organization using existing method
                    user = await self.get_user_by_email(org_id, email)
                    if user:
                        self.logger.debug("Global user search found user", 
                                        email=email, 
                                        user_id=user.id, 
                                        org_id=org_id)
                        return user
                    ...
                    # Log but continue searching in other orgs
                    self.logger.debug("Error searching in org", org_id=org_id, email=email, error=str(e))
                    continue
    ...
    async def update_user_last_login(self, org_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error("Error updating user last login", org_id=org_id, user_id=user_id, error=str(e))
            raise
