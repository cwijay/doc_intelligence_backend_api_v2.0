...
from app.core.security import (
    ...
    create_refresh_token, create_user_token_data,
    generate_invitation_token, verify_invitation_token
)
from app.services.user_service import user_service, UserNotFoundError, UserAlreadyExistsError
from app.services.org_service import organization_service, OrganizationAlreadyExistsError
...
class AuthService:
    ...
    async def authenticate_user(self, email: str, password: str) -> Tuple[str, str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not user.is_active:
                self.logger.warning("Authentication failed - user inactive", email=email, user_id=user.id)
                raise UserInactiveError("User account is inactive")

            # Check if organization is active
            org = await organization_service.get_organization(user.org_id)
            if not org.is_active:
                self.logger.warning("Authentication failed - organization inactive", email=email, org_id=user.org_id)
            ...
            token_data = create_user_token_data(user.id, user.org_id, user.email, user.role)
            ...
            if settings.INVALIDATE_TOKENS_ON_LOGIN:
                invalidated_count = invalidate_user_sessions(user.id, user.org_id, access_token_info.token_id)
                self.logger.info("Invalidated previous user sessions", 
                               user_id=user.id,
                               invalidated_count=invalidated_count)
            
            # Create refresh token with enterprise tracking
            refresh_token, refresh_token_info = create_refresh_token(user.id, user.org_id)
            
            self.logger.info("Authentication tokens created successfully", 
            ...
            return access_token, refresh_token, {
                ...
                "access_token_expires_in": int((access_token_info.expires_at - access_token_info.issued_at).total_seconds()),
                "refresh_token_expires_in": int((refresh_token_info.expires_at - refresh_token_info.issued_at).total_seconds())
            }
            ...
            self.logger.error("Authentication failed - unexpected error",
                            email=email, 
                            error=str(e),
    ...
    async def register_user_with_organization(
        ...
        try:
            self.logger.info("Starting user registration", 
                           email=email, org_name=organization_name)

            # Check if organization already exists
            existing_org = await organization_service.get_organization_by_name(organization_name)
            
            if existing_org:
                # Organization exists - add user to existing organization
                organization = existing_org
                user_role = UserRole.USER  # New users join as regular users
                self.logger.info("Adding user to existing organization", 
                               email=email, org_id=organization.id, org_name=organization_name)
            else:
                ...
                self.logger.info("Created new organization and adding user as admin", 
                               email=email, org_id=organization.id, org_name=organization_name)
            ...
            self.logger.info("User registration completed successfully", 
                           email=email, user_id=user.id, org_id=organization.id, role=user_role.value)
            ...
            self.logger.warning("User already exists in organization", 
                              email=email, org_name=organization_name, error=str(e))
            raise RegistrationError(f"User with this email or username already exists in '{organization_name}'. Please use different credentials or try logging in.")
        except Exception as e:
    ...
    async def register_user_with_invitation(
        ...
        try:
            ...
            self.logger.info("Processing invitation-based registration", 
                           email=email, org_id=org_id, role=role.value)
            ...
            self.logger.info("Invitation-based registration completed", 
                           email=email, user_id=user.id, org_id=org_id)
    ...
    async def create_invitation_token(
        ...
        try:
            ...
            self.logger.info("Invitation token created", 
                           org_id=org_id, email=email, role=role.value)
            ...
            self.logger.error("Error creating invitation", org_id=org_id, email=email, error=str(e))
            raise InvitationError("Failed to create invitation")

    async def refresh_access_token(self, refresh_token: str) -> Tuple[str, Optional[str], Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.info("Token refresh completed", 
                           user_id=user_id,
                           org_id=org_id,
                           new_access_token_id=access_token_info.token_id[:8] + "...",
                           new_refresh_token_id=refresh_token_info.token_id[:8] + "..." if refresh_token_info else "not_rotated")
    ...
    async def authenticate_user_simple(self, email: str, password: str) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.info("Simple authentication completed successfully", 
                           email=email, 
                           user_id=user.id, 
                           org_id=user.org_id,
                           role=user.role,
            ...
            # Unexpected errors - log detailed info for debugging
            self.logger.error("Simple authentication failed - unexpected error",
                            email=email, 
                            error=str(e),
                            error_type=type(e).__name__,
                            exc_info=True)
    ...
    async def register_user_simple(
        ...
        """
        try:
            self.logger.info("Starting simple MVP registration", 
            ...
            try:
                ...
                self.logger.error("Organization verification failed", 
                                org_id=organization_id, error=str(e))
            ...
            self.logger.info("Simple MVP registration completed successfully", 
                           email=email, user_id=user.id, org_id=organization_id, role=user.role)
            ...
            self.logger.warning("User already exists", 
                              email=email, org_id=organization_id, error=str(e))
            raise RegistrationError(f"User with this email or username already exists in this organization. Please use different credentials or try logging in.")
