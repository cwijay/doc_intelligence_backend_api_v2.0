...
class OrganizationService:
    ...
    async def create_organization(
        ...
        try:
            ...
            if existing_org and existing_org.is_active:
                raise OrganizationAlreadyExistsError(f"Organization with name '{org_data.name}' already exists")
    ...
    async def get_organization(
        ...
        try:
            ...
            self.logger.error("Error retrieving organization", org_id=org_id, error=str(e))
            raise
    ...
    async def update_organization(
        ...
        try:
            ...
            if update_data.name and update_data.name != org.name:
                ...
                if existing_org and existing_org.id != org_id and existing_org.is_active:
                    raise OrganizationAlreadyExistsError(f"Organization with name '{update_data.name}' already exists")
            ...
            self.logger.info("Organization updated", org_id=org_id, updates=list(update_fields.keys()))
            ...
            self.logger.error("Error updating organization", org_id=org_id, error=str(e))
            raise
        ...
        try:
            ...
            if filters:
                if filters.plan_type:
                    firestore_filters.append(FieldFilter("plan_type", "==", filters.plan_type.value))
            ...
            async for doc in docs:
                ...
                if filters:
                    if filters.name and filters.name.lower() not in org.name.lower():
                        continue
                    if filters.domain and org.domain and filters.domain.lower() not in org.domain.lower():
                        continue
            ...
            org_responses = [OrganizationResponse.model_validate(org) for org in paginated_orgs]

            # Calculate pagination info
            total_pages = math.ceil(total / pagination.per_page) if total > 0 else 0

            self.logger.debug("Organizations listed", 
    ...
    async def delete_organization(
        ...
        try:
            ...
            self.logger.error("Error deleting organization", org_id=org_id, error=str(e))
            raise
    ...
    async def get_organization_by_name(
        ...
        try:
            ...
            self.logger.error("Error getting organization by name", name=name, error=str(e))
            raise
