...
class DocumentService:
    ...
    async def _enrich_document_metadata(self, document: Document) -> Document:
        ...
        try:
            ...
            if not document.created_at:
                ...
                self.logger.debug(
                    "Set missing created_at timestamp", document_id=document.id
                )

            if not document.updated_at:
                document.updated_at = document.created_at
                self.logger.debug(
                    "Set missing updated_at timestamp", document_id=document.id
                )
            ...
            if not isinstance(document.metadata, dict):
                document.metadata = {}
                self.logger.debug(
                    "Reset invalid metadata to empty dict", document_id=document.id
                )
            ...
            if not document.uploaded_by or not document.uploaded_by.strip():
                ...
                self.logger.warning("Set missing uploaded_by", document_id=document.id)

            return document

        except Exception as e:
            self.logger.error(
                "Error enriching document metadata",
    ...
    async def _list_documents_from_gcs(
        ...
        try:
            ...
            self.logger.info(
                "Listing files directly from GCS", org_id=org_id, gcs_path=gcs_path
            )
            ...
            for blob in blobs:
                ...
                try:
                    ...
                    self.logger.warning(
                        "Failed to convert blob to document",
                        blob_name=blob.name,
                        error=str(e),
                    )
                    continue
        ...
        try:
            ...
            if gcs_client.is_initialized:
                try:
                    ...
                    self.logger.error(
                        "Error fetching GCS files for sync validation",
                        org_id=org_id,
                        error=str(e),
                    )
            ...
            for doc in firestore_docs:
                if doc.storage_path:
                    ...
                    storage_path_counts[doc.storage_path] = (
                        storage_path_counts.get(doc.storage_path, 0) + 1
                    )
            ...
            duplicate_storage_paths = {
                path: count for path, count in storage_path_counts.items() if count > 1
            }

            # Find orphaned GCS files
            firestore_paths = {
                doc.storage_path for doc in firestore_docs if doc.storage_path
            }
                ...
                issues.append(
                    {
                        ...
                        "documents": [
                            ...
                            for doc in orphaned_firestore[:5]
                        ],  # Show first 5
                    }
                )
                recommendations.append(
                    "Remove orphaned Firestore documents using cleanup script"
                )
                ...
                issues.append(
                    {
                        ...
                        "paths": list(duplicate_storage_paths.keys())[
                            :5
                        ],  # Show first 5
                    }
                )
                recommendations.append(
                    "Remove duplicate Firestore documents with same storage_path"
                )
            ...
            if orphaned_gcs:
                ...
                recommendations.append(
                    "Create Firestore metadata for orphaned GCS files or delete unused files"
                )

            # Check for documents in UPLOADING status (stuck uploads)
            uploading_docs = [
                doc for doc in firestore_docs if doc.status == "uploading"
            ]
            if uploading_docs:
                issues.append(
                    {
                ...
                recommendations.append(
                    "Review stuck uploads and either complete or cancel them"
                )
    ...
    async def create_document(
        ...
        try:
            ...
            storage_path = await self._ensure_unique_storage_path(
        ...
        try:
            ...
            if filters and filters.folder_path and "/original/" in filters.folder_path:
                ...
                return await self._list_documents_from_gcs(
                    org_id, filters.folder_path, pagination, filters
                )
            ...
            async for doc in all_docs:
                try:
                    ...
                    if filters and filters.folder_path:
                        document_folder = self._extract_folder_from_storage_path(
                            document.storage_path
                        )
                        if (
                            not document_folder
    ...
    async def update_document_summary(
        ...
        try:
            ...
            self.logger.error(
                "Error updating document summary",
                org_id=org_id,
                document_id=document_id,
                error=str(e),
            )
    ...
    async def update_document_ai_content(
        ...
        try:
            ...
            if not updates:
                self.logger.warning(
                    "No AI content updates provided",
                    org_id=org_id,
                    document_id=document_id,
                )
            ...
            self.logger.error(
                "Error updating document AI content",
                org_id=org_id,
                document_id=document_id,
                error=str(e),
            )
    ...
    async def get_document_ai_content(
        ...
        try:
            ...
            self.logger.error(
                "Error getting document AI content",
                org_id=org_id,
                document_id=document_id,
                error=str(e),
            )
        ...
        try:
            ...
            if gcs_client.is_initialized:
                try:
                    ...
                    self.logger.info(
                        "Document file deleted from GCS",
                        org_id=org_id,
                        document_id=document_id,
                        storage_path=document.storage_path,
                    )
    ...
    async def get_documents_by_folder_name_from_firestore(
        ...
        try:
            ...
            self.logger.info(
                "Found folder for document search",
                org_id=org_id,
                folder_name=folder_name,
                folder_id=folder.id,
            ...
            for attempt_folder_id in potential_ids:
                ...
                query = collection.where(
                    filter=FieldFilter("folder_id", "==", attempt_folder_id)
                )
                ...
                if additional_filters:
                    ...
                    if additional_filters.uploaded_by:
                        query = query.where(
                            filter=FieldFilter(
                                "uploaded_by", "==", additional_filters.uploaded_by
                            )
                        )
                ...
                async for doc in docs:
                    try:
                        ...
                        self.logger.error(
                            "Failed to process document during folder search",
                            org_id=org_id,
                            folder_name=folder_name,
                            folder_id_attempt=attempt_folder_id,
                            document_id=doc.id,
            ...
            if len(all_documents) == 0:
                self.logger.info(
                    "No documents found with folder ID patterns, trying fallback direct name search",
                    org_id=org_id,
                    folder_name=folder_name,
                ...
                fallback_patterns = [
                    p for p in fallback_patterns if p not in potential_ids
                ]

                for fallback_pattern in fallback_patterns:
                    self.logger.info(
                        "Trying fallback folder_id pattern",
                        org_id=org_id,
                    ...
                    # Base query - filter by folder_id
                    query = collection.where(
                        filter=FieldFilter("folder_id", "==", fallback_pattern)
                    )
                    ...
                    async for doc in docs:
                        try:
                            ...
                            self.logger.error(
                                "Failed to process document during fallback search",
                                org_id=org_id,
                                folder_name=folder_name,
                                fallback_pattern=fallback_pattern,
                                document_id=doc.id,
            ...
            self.logger.info(
                "Final Firestore query results for folder documents",
                org_id=org_id,
                folder_name=folder_name,
                folder_id_primary=folder.id,
                documents_found=len(all_documents),
                document_ids=[doc.id[:8] + "..." for doc in all_documents[:5]],
            )  # Show first 5 IDs
            ...
            self.logger.info(
                "Documents found via Firestore folder search",
                org_id=org_id,
                folder_name=folder_name,
                folder_id=folder.id,
    ...
    async def _find_folder_by_name(
        ...
        try:
            ...
            while page <= max_pages:
                ...
                )  # Get folders (max allowed)
                filters = FolderFilters(name=folder_name) if exact_match else None

                folder_list = await self.folder_service.list_folders(
                    org_id, pagination, filters
                )
                ...
                if (
                    exact_match and matching_folders
                ) or page >= folder_list.total_pages:
                    break
            ...
            if len(matching_folders) > 1:
                self.logger.warning(
                    "Multiple folders found with same name",
                    org_id=org_id,
                    folder_name=folder_name,
            ...
            class SimpleFolder:
                def __init__(self, folder_response):
                    ...
                    self.parent_folder_id = getattr(
                        folder_response, "parent_folder_id", None
                    )
                    self.created_by = getattr(folder_response, "created_by", "unknown")
                    self.created_at = getattr(
                        folder_response, "created_at", datetime.utcnow()
                    )
                    ...
                    self.potential_folder_ids = [
                        x
                        for x in self.potential_folder_ids
    ...
    async def sync_content_to_firestore(
        ...
        try:
            ...
            if not firebase_manager.is_initialized:
                self.logger.warning(
                    "Firebase not initialized, cannot sync content to Firestore"
                )
                return {
                    "success": False,
            ...
            async for doc in docs:
                ...
                updated_documents.append(
                    {"document_id": doc.id, "storage_path": storage_path}
                )

                self.logger.info(
                    "Synced content to Firestore document",
                    org_id=org_id,
                    document_id=doc.id,
                    storage_path=storage_path,
            ...
            if not document_found:
                raise DocumentNotFoundError(
                    f"No document found with storage path: {storage_path}"
                )
            ...
            self.logger.error(
                "Failed to sync content to Firestore",
                org_id=org_id,
                storage_path=storage_path,
                error=str(e),
            )
            return {"success": False, "error": str(e), "document_updated": False}

    async def sync_content_from_firestore_to_gcs(
        self, org_id: str, document_id: str, gcs_storage_path: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not gcs_storage_path:
                ...
                if not gcs_storage_path:
                    ...
                    if original_path:
                        gcs_storage_path = self._generate_parsed_storage_path(
                            original_path
                        )
                    else:
                        return {
            ...
            try:
                ...
                self.logger.info(
                    "Synced content from Firestore to GCS",
                    org_id=org_id,
                    document_id=document_id,
            ...
            self.logger.error(
                "Failed to sync content from Firestore to GCS",
                org_id=org_id,
                document_id=document_id,
                error=str(e),
            )
            return {"success": False, "error": str(e), "gcs_updated": False}
        ...
        try:
            ...
            if firestore_content == gcs_content:
                return {
                    "valid": True,
                    "synchronized": True,
                    "firestore_content_length": (
                        len(firestore_content) if firestore_content else 0
                    ),
            ...
            self.logger.error(
                "Failed to validate content sync",
                org_id=org_id,
                document_id=document_id,
                storage_path=storage_path,
                error=str(e),
