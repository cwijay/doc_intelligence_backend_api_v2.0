...
def handle_document_not_found_error(e: DocumentNotFoundError, operation: str, **context) -> HTTPException:
    """Handle DocumentNotFoundError consistently across endpoints."""
    logger.warning(f"Document not found for {operation}",
                  error=str(e),
                  **context)
...
def handle_document_validation_error(e: DocumentValidationError, operation: str, **context) -> HTTPException:
    """Handle DocumentValidationError consistently across endpoints."""
    logger.warning(f"Document validation failed for {operation}",
                  error=str(e),
                  **context)
    return HTTPException(
...
def handle_document_upload_error(e: DocumentUploadError, operation: str, **context) -> HTTPException:
    """Handle DocumentUploadError consistently across endpoints."""
    logger.error(f"Document upload error during {operation}",
                error=str(e),
                **context)
    return HTTPException(
