...
async def create_user(
    ...
    """
    try:
        logger.info("Creating user", org_id=org_id, email=user.email, username=user.username)
        ...
        logger.warning("User creation failed - organization not found", 
                      org_id=org_id, 
                      error=str(e))
        raise HTTPException(
        ...
        logger.warning("User creation failed - user exists", 
                      org_id=org_id, 
                      email=user.email,
...
async def get_user(
    ...
    try:
        ...
        logger.warning("User retrieval failed - organization not found", 
                      org_id=org_id, 
                      user_id=user_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_user(
    ...
    try:
        ...
        logger.warning("User update failed - organization not found", 
                      org_id=org_id, 
                      user_id=user_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
        ...
        logger.warning("User update failed - user not found", 
                      org_id=org_id, 
                      user_id=user_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
        ...
        logger.warning("User update failed - conflict", 
                      org_id=org_id, 
                      user_id=user_id,
                      error=str(e))
        raise HTTPException(
...
async def list_users(
    ...
    username: str = Query(default=None, description="Filter by username (partial match)"),
    full_name: str = Query(default=None, description="Filter by full name (partial match)"),
...
async def delete_user(
    ...
    try:
        ...
        if success:
            ...
            logger.error("User deletion failed unexpectedly", org_id=org_id, user_id=user_id)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.warning("User deletion failed - organization not found", 
                      org_id=org_id, 
                      user_id=user_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_user_by_email(
    ...
    try:
        ...
        logger.warning("User search by email failed - organization not found", 
                      org_id=org_id, 
                      email=email)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_user_by_username(
    ...
    try:
        ...
        if result is None:
            logger.warning("User not found by username", org_id=org_id, username=username)
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        logger.warning("User search by username failed - organization not found", 
                      org_id=org_id, 
                      username=username)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_user_stats(
    ...
    try:
        ...
        # Count by role
        role_counts = {
            "admin": sum(1 for user in all_users.users if user.role == UserRole.ADMIN and user.is_active),
            "user": sum(1 for user in all_users.users if user.role == UserRole.USER and user.is_active),
            "viewer": sum(1 for user in all_users.users if user.role == UserRole.VIEWER and user.is_active)
        }
