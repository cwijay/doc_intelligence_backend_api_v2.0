                ...
                "application/json": {
                    "examples": {
                        "invalid_file_type": {
                            ...
                            "value": {
                                "detail": "Only PDF and XLSX files are allowed"
                            }
                        },
                        "file_too_large": {
...
async def upload_document(
    ...
    current_user: Dict[str, Any] = Depends(get_current_user_dict)  # TEMPORARY: Mock auth for testing
):
    """
    ...
    **Path Priority:**
    ...
    logger.info("🔐 SESSION AUTHENTICATION INFO", 
...
async def get_download_url(
    ...
    try:
        ...
        logger.warning("Document not found for download", 
                      org_id=org_id,
                      document_id=document_id,
                      error=str(e))
        raise HTTPException(
        ...
        logger.warning("Document download validation failed", 
                      org_id=org_id,
                      document_id=document_id,
                      error=str(e))
        raise HTTPException(
...
async def download_document_redirect(
    ...
    try:
        ...
        logger.info("Generating direct download redirect", 
                   org_id=org_id,
                   document_id=document_id)
        ...
        logger.warning("Document not found for download redirect", 
                      org_id=org_id,
                      document_id=document_id,
                      error=str(e))
        raise HTTPException(
        ...
        logger.error("Error generating download redirect", 
                    org_id=org_id,
                    document_id=document_id,
                    error=str(e))
        raise HTTPException(
...
async def update_document_status(
    ...
    try:
        ...
        logger.warning("Document status update validation failed", 
                      org_id=org_id,
                      document_id=document_id,
                      error=str(e))
        raise HTTPException(
...
async def validate_document_sync(
    ...
    **Use Cases:**
    ...
    try:
        ...
        logger.info("Validating document sync", 
                   org_id=org_id,
                   folder_id=folder_id)
        ...
        logger.info("Document sync validation completed", 
                   org_id=org_id,
                   folder_id=folder_id,
        ...
        logger.error("Error during sync validation", 
                    org_id=org_id,
                    folder_id=folder_id,
                    error=str(e))
        raise HTTPException(
...
async def get_document_by_filename_firestore(
    ...
    **Key Benefits over GCS-based approach:**
    ...
    from app.models.schemas import DocumentFirestoreResponse
    
    try:
        # Get user/org info from session
        org_id = current_user["org_id"]
        
        logger.info("Firestore filename search requested", 
                   org_id=org_id,
                   filename=filename,
        ...
        logger.info("Firestore filename search completed successfully", 
                   org_id=org_id,
                   filename=filename,
        ...
        logger.warning("Document not found in Firestore filename search", 
                      org_id=org_id,
                      filename=filename,
                      exact_match=exact_match,
                      error=str(e))
        ...
        logger.error("Error in Firestore filename search", 
                    org_id=org_id,
                    filename=filename,
                    exact_match=exact_match,
                    error=str(e))
...
async def list_documents_by_folder_name_firestore(
    ...
    **Key Benefits over GCS bucket iteration:**
    ...
    """
    # Import here to avoid circular imports
    from app.models.schemas import DocumentFirestoreFolderListResponse, PaginationParams, DocumentFilters
    
    try:
        # Get user/org info from session
        org_id = current_user["org_id"]
        
        logger.info("Firestore folder document listing requested", 
                   org_id=org_id,
        ...
        logger.info("Firestore folder document listing completed successfully", 
                   org_id=org_id,
                   folder_name=folder_name,
        ...
        logger.warning("Folder not found in Firestore folder search", 
                      org_id=org_id,
                      folder_name=folder_name,
                      exact_match=exact_match,
                      error=str(e))
        ...
        logger.error("Error in Firestore folder document listing", 
                    org_id=org_id,
                    folder_name=folder_name,
                    exact_match=exact_match,
                    error=str(e))
...
async def parse_document(
    ...
    try:
        ...
        logger.info("Document parsing requested", 
                   org_id=org_id,
                   user_id=user_id,
        ...
        logger.warning("Unsupported file type for parsing", 
                      org_id=org_id,
                      storage_path=parse_request.storage_path,
                      error=str(e))
        raise HTTPException(
        ...
        logger.warning("Document not found for parsing", 
                      org_id=org_id,
                      storage_path=parse_request.storage_path,
                      error=str(e))
        raise HTTPException(
...
async def get_parsed_document(
    ...
    try:
        ...
        logger.error("Error checking parsed document", 
                    org_id=org_id,
                    storage_path=storage_path,
                    error=str(e))
        raise HTTPException(
