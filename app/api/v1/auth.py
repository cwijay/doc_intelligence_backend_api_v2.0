...
from app.models.schemas import OrganizationResponse, PaginationParams, OrganizationFilters
...
class RegisterRequest(BaseModel):
    ...
    organization_name: Optional[str] = Field(None, description="Deprecated: use organization_id instead")
    domain: Optional[str] = Field(None, description="Deprecated: not used in MVP")
    plan_type: Optional[PlanType] = Field(None, description="Deprecated: not used in MVP")
...
class AuthResponse(BaseModel):
    ...
    access_token_expires_at: str = Field(..., description="Access token expiration timestamp (ISO format)")
    refresh_token_expires_at: str = Field(..., description="Refresh token expiration timestamp (ISO format)")
...
class AccessTokenResponse(BaseModel):
    ...
    refresh_token: Optional[str] = Field(None, description="New refresh token if rotation is enabled")
    token_type: str = Field(default="bearer", description="Token type")
    expires_in: int = Field(..., description="Access token expiration in seconds")
    access_token_expires_at: str = Field(..., description="Access token expiration timestamp (ISO format)")
    refresh_token_expires_at: Optional[str] = Field(None, description="Refresh token expiration timestamp if rotated")
    refresh_token_expires_in: Optional[int] = Field(None, description="Refresh token expiration in seconds if rotated")
    rotation_enabled: bool = Field(..., description="Whether token rotation is enabled")
    refresh_token_rotated: bool = Field(..., description="Whether refresh token was rotated")
...
async def login(request: LoginRequest) -> AuthResponse:
    ...
    try:
        ...
        return AuthResponse(
            ...
            refresh_token=session.refresh_token or session.session_id,  # Refresh token or fallback
            expires_in=session.time_until_expiry(),
            refresh_expires_in=int((session.refresh_expires_at - datetime.utcnow()).total_seconds()) if session.refresh_expires_at else session.time_until_expiry(),
            access_token_expires_at=session.expires_at.isoformat(),
            refresh_token_expires_at=session.refresh_expires_at.isoformat() if session.refresh_expires_at else session.expires_at.isoformat(),
            user={
                "user_id": user_data["user_id"],
                "email": user_data["email"],
                "full_name": user_data["full_name"],
    ...
    except OrganizationInactiveError as e:
...
async def register(request: RegisterRequest) -> AuthResponse:
    ...
    """
    try:
        logger.info("MVP registration attempt", 
                   email=request.email, 
        ...
        logger.info("MVP registration successful", 
                   email=request.email, 
                   user_id=user_data["user_id"],
                   org_id=user_data["org_id"])
...
async def register_with_invitation(request: InviteRegisterRequest) -> AuthResponse:
    ...
    """
    try:
        logger.info("Invitation registration attempt", token=request.invitation_token[:10] + "...")
        
        access_token, refresh_token, user_data = await auth_service.register_user_with_invitation(
        ...
        logger.info("Invitation registration successful", 
                   email=user_data["email"],
                   user_id=user_data["user_id"],
                   org_id=user_data["org_id"])
...
async def refresh_access_token(request: RefreshTokenRequest) -> AccessTokenResponse:
    ...
    try:
        ...
        access_token, new_refresh_token, token_info = await auth_service.refresh_access_token(request.refresh_token)
...
async def logout(
    ...
    try:
        ...
        if not success:
            logger.warning("Failed to invalidate session - session not found", 
                         user_id=user_id,
                         session_id=session_id[:8] + "..." if session_id else "unknown")
...
async def logout_all_sessions(
    ...
    try:
        ...
        logger.info("🔒 LOGOUT-ALL request received - Security action", 
                   user_id=user_id,
                   org_id=org_id)
        
        if not all([user_id, org_id]):
            logger.error("Invalid user data for logout-all", 
        ...
        logger.info("🔒 LOGOUT-ALL completed successfully", 
                   user_id=user_id, 
                   org_id=org_id,
...
async def create_invitation(
    ...
    try:
        ...
        logger.warning("Invitation creation failed", org_id=org_id, email=email, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
    except Exception as e:
        logger.error("Invitation creation error", org_id=org_id, email=email, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_organizations_for_registration() -> List[OrganizationResponse]:
    ...
    try:
        ...
        pagination = PaginationParams(page=1, per_page=100)  # Get up to 100 orgs for selection
...
async def lookup_organizations(
    query: str = Query(..., description="Search query for organization name", min_length=1)
    ...
    try:
        ...
        logger.debug("Organization lookup completed",
                    query=query,
...
async def check_organization_availability(
    ...
    try:
        ...
        logger.debug("Organization availability check completed", 
                    name=name, 
...
async def refresh_session_token(
    ...
    try:
        logger.info("Session refresh attempt", 
                   refresh_token=request.refresh_token[:8] + "...")
        ...
        refresh_expires_in = int((new_session.refresh_expires_at - datetime.utcnow()).total_seconds()) if new_session.refresh_expires_at else 0
        ...
        return AuthResponse(
            ...
            refresh_token_expires_at=new_session.refresh_expires_at.isoformat() if new_session.refresh_expires_at else "",
            user={
                "user_id": new_session.user_id,
    ...
    try:
        ...
        try:
            with simple_auth_manager._lock:
                session_exists = session_id in simple_auth_manager._sessions if session_id else False
                active_session_count = simple_auth_manager.get_active_session_count()
                user_session_count = simple_auth_manager.get_user_session_count(user_id, org_id) if user_id and org_id else 0
                ...
                if session_exists and session_id:
                    ...
                    diagnostic_info["session_details"] = {
                        ...
                        "refresh_expires_at": session.refresh_expires_at.isoformat() if session.refresh_expires_at else None
                    }
        ...
        logger.info("🔍 SESSION DIAGNOSTICS completed", 
                   session_id=session_id[:8] + "..." if session_id else "N/A",
                   user_id=user_id,
