...
async def create_organization(
    ...
    try:
        ...
        logger.warning("Organization creation failed - name exists", 
...
async def update_organization(
    ...
    try:
        ...
        logger.warning("Organization update failed - name conflict", 
                      org_id=org_id, 
                      error=str(e))
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
async def list_organizations(
    ...
    name: str = Query(default=None, description="Filter by organization name (partial match)"),
    ...
    try:
        ...
        logger.debug("Organizations listed successfully", 
...
async def get_organization_by_name(
    ...
    try:
        ...
        logger.error("Failed to retrieve organization by name", 
                    name=name, 
                    error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_organization_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        # Count by plan type
        plan_counts = {
            "free": sum(1 for org in all_orgs.organizations if org.plan_type == PlanType.FREE and org.is_active),
            "starter": sum(1 for org in all_orgs.organizations if org.plan_type == PlanType.STARTER and org.is_active),
            "pro": sum(1 for org in all_orgs.organizations if org.plan_type == PlanType.PRO and org.is_active)
        }
        
        # Count organizations with domains
        with_domain_count = sum(1 for org in all_orgs.organizations if org.domain and org.is_active)
